...
{
	[TestFixture]
	public class RandomBinaryConventionTests
	{
		[Test]
		public void Apply()
		{
			Mock<ITypePropertyConventionContext> contextMock = new Mock<ITypePropertyConventionContext>();
			var target = new RandomBinaryConvention();

			target.Apply(contextMock.Object);

			contextMock.Verify( a => a.SetSource<RandomBinaryDataSource>(), @"
...
");
		...
		public void IntegrationTest()
		{
			var mFactory = AutoPocoContainer.Configure(x =>
			{
				x.Conventions(c =>
				{
					c.Register<RandomBinaryConvention>();
			...
			var targets = session.List<TestClass>(100).Get()
				.Select(a => a.TargetProperty == null ? string.Empty : System.Text.Encoding.Default.GetString(a.TargetProperty));
