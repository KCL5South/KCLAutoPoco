...
{
	[TestFixture]
	public class RandomNullableBooleanConventionTests
	{
		[Test]
		public void Apply()
		{
			Mock<ITypePropertyConventionContext> contextMock = new Mock<ITypePropertyConventionContext>();
			var target = new RandomNullableBooleanConvention();

			target.Apply(contextMock.Object);

			contextMock.Verify( a => a.SetSource<RandomNullableBooleanDataSource>(), @"
...
");
		...
		public void SpecifyRequirements()
		{
			...
			reqMock.Verify(a => a.Type(x => x == typeof(System.Nullable<bool>)), @"
...
");
		...
		public void IntegrationTest()
		{
			var mFactory = AutoPocoContainer.Configure(x =>
			{
				x.Conventions(c =>
				{
					c.Register<RandomNullableBooleanConvention>();
