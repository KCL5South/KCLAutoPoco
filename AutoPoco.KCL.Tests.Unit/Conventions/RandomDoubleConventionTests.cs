...
{
	[TestFixture]
	public class RandomDoubleConventionTests
	{
		[Test]
		public void Apply()
		{
			Mock<ITypePropertyConventionContext> contextMock = new Mock<ITypePropertyConventionContext>();
			var target = new RandomDoubleConvention();

			target.Apply(contextMock.Object);

			contextMock.Verify( a => a.SetSource<RandomDoubleDataSource>(), @"
...
");
		...
		public void SpecifyRequirements()
		{
			...
			reqMock.Verify(a => a.Type(x => x == typeof(double)), @"
...
");
		...
		public void IntegrationTest()
		{
			var mFactory = AutoPocoContainer.Configure(x =>
			{
				x.Conventions(c =>
				{
					c.Register<RandomDoubleConvention>();
