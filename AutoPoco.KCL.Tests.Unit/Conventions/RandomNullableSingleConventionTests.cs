...
{
	[TestFixture]
	public class RandomNullableSingleConventionTests
	{
		[Test]
		public void Apply()
		{
			Mock<ITypePropertyConventionContext> contextMock = new Mock<ITypePropertyConventionContext>();
			var target = new RandomNullableSingleConvention();

			target.Apply(contextMock.Object);

			contextMock.Verify( a => a.SetSource<RandomNullableSingleDataSource>(), @"
...
");
		...
		public void SpecifyRequirements()
		{
			...
			reqMock.Verify(a => a.Type(x => x == typeof(System.Nullable<float>)), @"
...
");
		...
		{
			public float? TargetProperty { get; set; }
		}
		...
		public void IntegrationTest()
		{
			var mFactory = AutoPocoContainer.Configure(x =>
			{
				x.Conventions(c =>
				{
					c.Register<RandomBooleanConvention>();
