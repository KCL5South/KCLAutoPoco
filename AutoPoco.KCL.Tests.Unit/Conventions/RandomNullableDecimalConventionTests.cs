...
{
	[TestFixture]
	public class RandomNullableDecimalConventionTests
	{
		[Test]
		public void Apply()
		{
			Mock<ITypePropertyConventionContext> contextMock = new Mock<ITypePropertyConventionContext>();
			var target = new RandomNullableDecimalConvention();

			target.Apply(contextMock.Object);

			contextMock.Verify( a => a.SetSource<RandomNullableDecimalDataSource>(), @"
...
");
		...
		public void SpecifyRequirements()
		{
			...
			reqMock.Verify(a => a.Type(x => x == typeof(System.Nullable<decimal>)), @"
...
");
		...
		public void IntegrationTest()
		{
			var mFactory = AutoPocoContainer.Configure(x =>
			{
				x.Conventions(c =>
				{
					c.Register<RandomNullableDecimalConvention>();
