...
{
	[TestFixture]
	public class RandomInt64ConventionTests
	{
		[Test]
		public void Apply()
		{
			Mock<ITypePropertyConventionContext> contextMock = new Mock<ITypePropertyConventionContext>();
			var target = new RandomInt64Convention();

			target.Apply(contextMock.Object);

			contextMock.Verify( a => a.SetSource<RandomInt64DataSource>(), @"
...
");
		...
		public void SpecifyRequirements()
		{
			...
			reqMock.Verify(a => a.Type(x => x == typeof(long)), @"
...
");
		...
		public void IntegrationTest()
		{
			var mFactory = AutoPocoContainer.Configure(x =>
			{
				x.Conventions(c =>
				{
					c.Register<RandomInt64Convention>();
