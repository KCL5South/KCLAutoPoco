...
{
	[TestFixture]
	public class RandomSingleDataSourceTests
	{
		[Test]
		public void Constructor_RandIsSet()
		{
			IRandomExtensions rand = Mock.Of<IRandomExtensions>();
			var target = new RandomSingleDataSource(rand);
		...
		public void Next()
		{
			...
			var target = new RandomSingleDataSource(randMock.Object);
			target.Next(null);

			randMock.Verify(a => a.GetSingle(System.Single.MinValue, System.Single.MaxValue), @"
The GetSingle method on the IRandomExtensions object was not called.
");
		}
	}
}