...
Is Doxygen available?
	If this test fails, it is because the doxygen.exe has not been included in this machine's
	'Path' environment variable.  
	
	Add the location of doxygen.exe to the machine's 'Path' environment variable and run
	this test again.
"@	{
		@($env:Path.Split(";") | where {
